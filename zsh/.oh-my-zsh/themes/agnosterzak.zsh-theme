...
prompt_context() {
  if [[ -n "$SSH_CLIENT" ]]; then
    ...
    prompt_segment 53 white "%{$fg_bold[white]%(!.%{%F{white}%}.)%}$USER@Archie%{$fg_no_bold[white]%}"
  fi
}
...
prompt_dir() {
  prompt_segment 93 white "%{$fg_bold[white]%}%~%{$fg_no_bold[white]%}"
}
...
prompt_time() {
  prompt_segment 129 white "%{$fg_bold[white]%}%D{%a %e %b - %H:%M}%{$fg_no_bold[white]%}"
}
